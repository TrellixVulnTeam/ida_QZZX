...
class FitSurrogatesTask(ComposableDataclass, LoggingMixin):
    ...
    class Results:
        ...
        def to_flat_pandas(self) -> pd.DataFrame:
            return pd.DataFrame({'cross_entropies': [score.cross_entropy for score in self.scores],
                                 'top_k_accuracies': [score.acc for score in self.scores],
                                 'influence_estimators': self.influence_estimators,
