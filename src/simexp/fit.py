...
from ida.common import NestedLogger, ComposableDataclass
from simexp.spark import Field, SparkSessionConfig


@dataclass
class TreeSurrogate(NestedLogger):
    ...
    def fit_and_score(self, X_train, y_train, X_test, y_test) -> Union[GridSearchCV, Score]:
        ...
        try:
            ...
            self.log_item('ERROR: The following exception occurred while scoring the model:\n{}'.format(str(e)))
            return cv
    ...
    def _score(self, cv: GridSearchCV, dummy: DummyClassifier, X_test, y_test) -> Score:
        ...
        try:
            ...
            self.log_item('ERROR: The following exception occurred while computing node counts:\n{} {}'
                          .format(type(e), e))
            counts = None
...
@dataclass
class SurrogatesFitter(ComposableDataclass, NestedLogger):
    """
    ...
    def _fit_and_score(self, influence_estimator: str, perturber: str, detector: str, perturb_fraction: Optional[float],
        ...
        with self.log_task('Fitting surrogate model based on:'):
            self.log_item('Influence estimator: {}'.format(influence_estimator))
            self.log_item('Perturber: {}'.format(perturber))
            self.log_item('Detector: {}'.format(detector))
            ...
            if influence_estimator is None:
                ...
                if perturb_fraction is not None and perturb_image_count < target_perturb_image_count:
                    self.log_item('WARNING: found not enough perturbations to create a training dataset '
            ...
            self.log_item('We have {} candidate observations.'.format(len(train_obs)))

            indices = train_obs.filter_for_split(self.tree.k_folds)
            remaining_count = np.count_nonzero(indices)
            if remaining_count > 0:
                self.log_item('After filtering for CV, {} observations remain.'.format(remaining_count))
            else:
                self.log_item('WARNING: There are not enough observations for a {}-fold CV.'
    ...
    def _run_for_single_train_sample(self, train_df: DataFrame, perturbed_df: DataFrame, all_concept_names: [str],
        ...
        with self.log_task('Selected hyperparameters for fitting:'):
            for group_no, group in enumerate(groups, start=1):
                with self.log_task('Combination {}'.format(group_no)):
                    for x in group:
                        self.log_item(x)
    ...
    def _train_df_sample_iter(self, train_df: DataFrame):
        if self.use_balanced_sampling:
            ...
            if len(below_threshold) > 0:
                with self.log_task('WARNING: the following classes have less observations than the required {}:'
                                    .format(max(self.train_observations_per_class))):
                    for class_name, class_count in below_threshold.items():
                        self.log_item('Class {} has {} observations.'.format(class_name, class_count))
    ...
    def run(self) -> Results:
        with self.log_task('Training surrogate models'):
            ...
            for concept_field in all_concept_fields:
                ...
                if concept_name not in train_df.columns:
                    self.log_item('Adding concept {} to training data'.format(concept_field.name))
                    train_df = train_df.withColumn(concept_name, sf.lit(0))

                if concept_name not in perturbed_df.columns:
                    self.log_item('Adding concept {} to perturbed data'.format(concept_field.name))
                    perturbed_df = perturbed_df.withColumn(concept_name, sf.lit(0))

                if concept_name not in test_df.columns:
                    self.log_item('Adding concept {} to test data'.format(concept_field.name))
            ...
            self.log_item('Using {} concepts in total (train + perturbations + test data).'
                          .format(len(all_concept_names)))

            test_obs = TestObservations(*self._decode(test_df, all_concept_names))
            test_df.unpersist()
            self.log_item('Test data comprises {} observations.'.format(len(test_obs)))
            ...
            for train_sample_no, (train_obs_per_class, train_df_sample) in enumerate(train_iter):
                with self.log_task('Running for training sample {}'.format(train_sample_no)):
